...
if getattr(sys, 'frozen', False):
    # PyInstaller ile build edilmişse
    DATA_FILE = os.path.join(sys._MEIPASS, DATA_FILE)
    # Runtime data klasörü (app yanında)
...
class SuTakipApp(ctk.CTk):
    ...
    def _musteri_satiri(self, m):
        ...
        ctk.CTkButton(satir, text="📦 Sipariş", width=74, height=28,
                      fg_color="#1d4ed8", hover_color="#1e40af",
                      font=ctk.CTkFont(size=11),
                      command=lambda ms=m: self._siparis_ver(ms)).pack(side="right", padx=(0, 4), pady=8)
        ctk.CTkButton(satir, text="🗑️ Sil", width=50, height=28,
                      fg_color="#ef4444", hover_color="#dc2626",
                      font=ctk.CTkFont(size=11),
                      command=lambda ms=m: self._musteri_sil(ms)).pack(side="right", padx=8, pady=8)
